...
def load_pending_gold(s3: Any, bucket: str, key: str) -> set[tuple[str, str]]:
    ...
    if not isinstance(partitions, list):
        return set()
    return {(str(item[0]), str(item[1])) for item in partitions if isinstance(item, (list, tuple)) and len(item) == 2}
...
def _aggregate(df: pd.DataFrame, keys: list[str]) -> pd.DataFrame:
    ...
    agrege = prepared.groupby(keys, dropna=False).agg(**GOLD_COUNTERS, **GOLD_METRICS).reset_index()
...
def aggregate_daily(df: pd.DataFrame) -> pd.DataFrame:
    ...
    daily["completeness_pct"] = (daily["covered_hours"] / EXPECTED_HOURS_PER_DAY * 100).round(1)
    return daily
...
def add_simple_anomalies(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[valid_std, "zscore_flag"] = (
        df.loc[valid_std, "consumption_kw"] - df.loc[valid_std, "site_mean"]
    ).abs() > 3 * df.loc[valid_std, "site_std"]
...
def rebuild_gold_partition(
    ...
    if "source_key" in partition_df.columns:
        ...
        partition_df = partition_df.sort_values(tri).drop_duplicates("source_key", keep="last").reset_index(drop=True)
...
def run_gold(argv: list[str] | None = None) -> GoldResult:
    ...
    try:
        for record_date, site_id in sorted(partitions):
            try:
                rebuild_gold_partition(s3, args.silver_bucket, args.gold_bucket, record_date, site_id, pg_conn=pg_conn)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if result.nouveaux == 0:
        print(
            f"Traitement terminé | objets_bronze={result.objets_bronze} | nouveaux=0 | lignes_silver=0 | quarantined=0"
        )
        return 0
