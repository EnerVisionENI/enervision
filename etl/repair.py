...
def partitions_a_rejouer(s3, gold_bucket, *, sites=None, debut=None, fin=None, workers=16):
    ...
    # Ordonné par SITE puis par date, et non l'inverse : les fenêtres de rétablissement
    # durent quelques dizaines de secondes, pendant lesquelles on veut enchaîner un maximum
    # de journées du même site. Trier par date fait alterner les sites et gaspille la
    # fenêtre à interroger des capteurs encore rouges.
    vides = {partition for partition in resultats if partition is not None}
    return sorted(vides, key=lambda partition: (partition[1], partition[0]))
...
def recuperer_journee(site_id, record_date, pas_minutes):
    ...
    try:
        debut = datetime.strptime(record_date, "%Y-%m-%d").replace(tzinfo=UTC)
    except ValueError:
