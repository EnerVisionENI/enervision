...
docker compose run --rm etl-collect python repair.py             # rejoue, puis :
docker compose run --rm etl-collect python quality.py
docker compose run --rm etl-collect python quality.py --gold-only
```

Il ne remplace une partition qu'après avoir un tirage non vide en main, et ne touche jamais
le bucket `audit` : l'empreinte de ce qui a réellement été reçu au premier passage reste
vérifiable.
